...
public class BlockUtil implements Wrapper {
    ...
    public static boolean isMining(BlockPos pos) {
        return Rebirth.BREAK.isMining(pos) || pos.equals(PacketMine.breakPos);
    }
    ...
    public static boolean hasEntity(BlockPos pos, boolean ignoreCrystal) {
        for (Entity entity : mc.world.getNonSpectatingEntities(Entity.class, new Box(pos))) {
            if (!entity.isAlive() || entity instanceof ItemEntity || entity instanceof ExperienceOrbEntity || entity instanceof ExperienceBottleEntity || entity instanceof ArrowEntity || ignoreCrystal && entity instanceof EndCrystalEntity || entity instanceof ArmorStandEntity && Rebirth.HUD.obsMode.getValue())
                continue;
            return true;
        }
        return false;
    ...
    public static boolean hasEntityBlockCrystal(BlockPos pos, boolean ignoreCrystal) {
        for (Entity entity : mc.world.getNonSpectatingEntities(Entity.class, new Box(pos))) {
            if (!entity.isAlive() || ignoreCrystal && entity instanceof EndCrystalEntity || entity instanceof ArmorStandEntity && Rebirth.HUD.obsMode.getValue())
                continue;
            return true;
        }
        return false;
    ...
    public static boolean hasEntityBlockCrystal(BlockPos pos, boolean ignoreCrystal, boolean ignoreItem) {
        for (Entity entity : mc.world.getNonSpectatingEntities(Entity.class, new Box(pos))) {
            if (!entity.isAlive() || ignoreItem && entity instanceof ItemEntity || ignoreCrystal && entity instanceof EndCrystalEntity || entity instanceof ArmorStandEntity && Rebirth.HUD.obsMode.getValue())
                continue;
            return true;
        }
        return false;
    ...
    public static Direction getPlaceSide(BlockPos pos) {
        return getPlaceSide(pos, Rebirth.HUD.placement.getValue() == Placement.Strict, Rebirth.HUD.placement.getValue() == Placement.Legit);
    }
    ...
    public static Direction getPlaceSide(BlockPos pos, double distance) {
        ...
        for (Direction i : Direction.values()) {
            if (canClick(pos.offset(i)) && !canReplace(pos.offset(i))) {
                if (Rebirth.HUD.placement.getValue() == Placement.Legit) {
                    if (!EntityUtil.canSee(pos.offset(i), i.getOpposite())) continue;
                } else if (Rebirth.HUD.placement.getValue() == Placement.Strict) {
                    if (!isStrictDirection(pos.offset(i), i.getOpposite())) continue;
                }
                double vecDis = mc.player.squaredDistanceTo(pos.toCenterPos().add(i.getVector().getX() * 0.5, i.getVector().getY() * 0.5, i.getVector().getZ() * 0.5));
    ...
    public static Direction getClickSide(BlockPos pos) {
        ...
        side = Direction.UP;
        for (Direction i : Direction.values()) {
            if (Rebirth.HUD.placement.getValue() == Placement.Strict) {
    ...
    public static ArrayList<BlockPos> getSphere(float range, Vec3d pos) {
        ...
        for (double x = pos.getX() - (range + 0.5); x < pos.getX() + (range + 0.5); ++x) {
            for (double y = pos.getY() - (range + 0.5); y < pos.getY() + (range + 0.5); ++y) {
                for (double z = pos.getZ() - (range + 0.5); z < pos.getZ() + (range + 0.5); ++z) {
                    BlockPos curPos = new BlockPosX(x, y, z);
                    if (MathHelper.sqrt((float) pos.squaredDistanceTo(curPos.toCenterPos())) <= range) {
                        list.add(curPos);
                    }
                }
            }
